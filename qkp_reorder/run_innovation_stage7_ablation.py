...
RESULT_PATH = RESULT_DIR / "ours_ablation_results.csv"
LOG_DIR = PROJECT_ROOT / "logs" / "innovation"
LOG_PATH = LOG_DIR / "stage7_ours_ablation_log.txt"

_DEFAULT_METHODS = "ours_bm25_only,ours_keyword,ours_full"
...
def _get_env_methods() -> List[str]:
    raw = os.getenv("STAGE7_METHODS", _DEFAULT_METHODS)
...
def main() -> None:
    ...
    logging.info("Methods to run: %s", methods)
    ...
    for sample in samples:
        ...
        for method in methods:
            ...
            if pair in completed_pairs:
                logging.info(
                    "[%s/%s] Skip completed: %s - %s",
                    current_count, total_planned, sample_id, method,
                )
                continue

            logging.info("=" * 40)
            logging.info("[%s/%s] Running method: %s", current_count, total_planned, method)
    ...
    logging.info("#" * 60)
    logging.info("Stage 7 ablation experiment finished.")
