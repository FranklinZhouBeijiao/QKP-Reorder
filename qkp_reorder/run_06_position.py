...
RESULT_DIR = PROJECT_ROOT / "results" / "stage6_position"
RESULT_PATH = RESULT_DIR / "exp2_position_results.csv"
...
def get_max_samples_to_run() -> int | None:
    """Read optional smoke-test sample limit from STAGE6_POSITION_MAX_SAMPLES."""
    value = os.getenv("STAGE6_POSITION_MAX_SAMPLES")
...
def load_existing_results() -> pd.DataFrame:
    """Load existing result CSV if present."""
...
def is_valid_completed_row(row: pd.Series) -> bool:
    """Only non-error, non-empty-prediction rows count as completed."""
...
def get_completed_pairs(df: pd.DataFrame) -> Set[Tuple[str, str]]:
    """Return valid completed (sample_id, method) pairs."""
...
def call_generate_answer_with_retry(prompt: str) -> Dict[str, Any]:
    """Generate answer and retry once if final content is empty."""
...
def run_one_method(
    ...
    """Run one sample-method pair and return a CSV row."""
...
def main() -> None:
    ...
    for sample in samples:
        ...
        print(f"Inferred answer position: {sample.get('inferred_answer_position')}")
    ...
    print("\n" + "#" * 100)
    print("Stage 6 position experiment finished.")
