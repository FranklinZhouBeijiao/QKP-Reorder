...
RESULT_PATH = RESULT_DIR / "ours_nq_100_results.csv"
LOG_DIR = PROJECT_ROOT / "logs" / "innovation"
LOG_PATH = LOG_DIR / "stage4_ours_nq_100_log.txt"
...
def _get_env_methods() -> List[str]:
    raw = os.getenv("STAGE4_METHODS", _DEFAULT_METHODS)
    return [m.strip() for m in raw.split(",") if m.strip()]


def get_max_samples_to_run() -> int | None:
    value = os.getenv("STAGE4_MAX_SAMPLES")
...
def main() -> None:
    ...
    for sample in samples:
        ...
        logging.info(f"Answer position: {sample.get('answer_position')}")
    ...
    logging.info("#" * 60)
    logging.info("Stage 4 NQ-100 innovation experiment finished.")
