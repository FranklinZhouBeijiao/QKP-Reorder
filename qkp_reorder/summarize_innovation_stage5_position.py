...
REPRO_RESULT_PATH = PROJECT_ROOT / "results" / "stage6_position" / "exp2_position_results.csv"
OURS_RESULT_PATH = PROJECT_ROOT / "results" / "innovation" / "ours_position_results.csv"
INNOVATION_DIR = PROJECT_ROOT / "results" / "innovation"
SUMMARY_PATH = INNOVATION_DIR / "ours_position_summary.csv"
COMPARISON_PATH = INNOVATION_DIR / "ours_position_comparison.csv"
...
def compute_position_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    nq_cols = ["nq_style_em", "nq_style_f1"]
    for col in nq_cols:
        if col in df.columns:
            agg_spec[f"{col}_avg"] = (col, "mean")
...
def print_position_comparison(summary: pd.DataFrame) -> None:
    """Print a focused comparison for positions 10, 15, 20."""
    target_positions = [10, 15, 20]
    target_methods = ["truncate", "bm25", "longllmlingua", "ours_full"]

    sub = summary[
        summary["answer_position"].isin(target_positions)
        & summary["method"].isin(target_methods)
    ]

    metrics = ["contains_answer_avg", "f1_avg"]
    if "nq_style_f1_avg" in sub.columns:
        metrics.append("nq_style_f1_avg")

    print("\n=== Key Comparison: Positions 10/15/20 ===")
    for metric in metrics:
        print(f"\n--- {metric} ---")
        pivot = sub.pivot_table(
            index="method", columns="answer_position", values=metric
        )
        pivot = pivot.reindex([m for m in target_methods if m in pivot.index])
        pivot = pivot.reindex(columns=target_positions)
...
def main() -> None:
    ...
    repro_df = load_and_label(REPRO_RESULT_PATH, "reproduction/stage6_position")
    ours_df = load_and_label(OURS_RESULT_PATH, "innovation/stage5")

    if repro_df is None and ours_df is None:
        raise FileNotFoundError("Neither reproduction nor innovation results found.")

    dfs: List[pd.DataFrame] = [d for d in [repro_df, ours_df] if d is not None]
    combined = pd.concat(dfs, ignore_index=True)

    combined = add_derived_columns(combined)
    combined = add_nq_style_metrics(combined)

    summary = compute_position_summary(combined)

    summary.to_csv(SUMMARY_PATH, index=False, encoding="utf-8-sig")
    print(f"[OK] Summary saved to: {SUMMARY_PATH}")

    comparison_cols = [
        "answer_position", "method", "sample_count", "valid_predictions",
    ...
    extra_nq = []
    for col in ["nq_style_em_avg", "nq_style_f1_avg"]:
        if col in summary.columns:
            extra_nq.append(col)
    comparison = summary[
        [c for c in comparison_cols + extra_nq if c in summary.columns]
    ]
    ...
    if ours_df is not None:
        print_position_comparison(summary)
