...
OUTPUT_PATH = PROJECT_ROOT / "data" / "stage6" / "processed" / "nq_multidoc_100.json"
...
def extract_documents(record: Dict[str, Any]) -> List[Dict[str, str]]:
    """Extract 10 input documents from a raw Lost-in-the-Middle record."""
    docs_raw = None
...
def infer_answer_position(
    ...
    """Infer 1-based answer position from raw flags or answer text matching."""
...
def convert_record(record: Dict[str, Any], index: int) -> Optional[Dict[str, Any]]:
    """Convert one raw record to project sample format."""
